...
public class Node : MonoBehaviour {
    ...
    public void Interact()
    {
        if (interactable == true)
        {
            ...
            switch (gm.currentPlayer.playerType)
            {
                case PlayerType.BLUE:
                    ...
                    if (PhotonNetwork.LocalPlayer.ActorNumber == 1)
                    {
                        ...
                        Debug.LogError("oper blue");
                        ShareValues.For_Node_no = 1;
          ...
          //  View.RPC(nameof(New_One), RpcTarget.AllBuffered);
            // View.RPC(nameof(gm.PlayWithChosenToken_func), RpcTarget.AllBuffered, chosenToken);

            //   gm.PlayWithChosenToken_func(chosenToken);
            //checking
        }
    }
    [PunRPC]
    ...
    {
        return transform.position + 3 * offset;
    }
